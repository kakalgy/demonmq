 ...
 *
 * This class can be used to split composite URI's into their component parts
 * and is used to extract any URI options from each URI in order to set specific
 * properties on Beans.
 * 
 * <p>
 * 
 * URL转换的工具类：可以将URI分割成不同的组件部分，能够将URI的Option分离出来，设置为Bean的Property
 * 
 * @author gyli
 *
 */
public class URISupport {
    /**
     * A composite(合成的) URI can be split into one or more CompositeData object which
     * each represent the individual URIs that comprise(组成) the composite one.
     * <p>
     * 合成URI可以分解为多个CompositeData
     * <p>
     * URI : https://www.cnblogs.com/ldq2016/p/7250399.html
     * <ul>
     ...
     * </ul>
     * URI的各个部分 ： http://jiangwenfeng762.iteye.com/blog/1621620
     */
    ...
    /**
     * Given a URI parse and extract any URI query options and return them as a Key
     * / Value mapping.
     *
     * This method differs from the {@link parseQuery} method in that it handles
     * composite URI types and will extract the URI options from the outermost
     * composite URI.
     *
     * @param uri
     *            The URI whose query should be extracted and processed.
     ...
     * represents the mapped entries, return the newly updated URI that contains the
     * value of the given URI and the appended query value.
     *
     * @param uri
     *            The source URI that will have the Map entries appended as a URI
     *            query value.
     * @param queryParameters
     *            The Key / Value mapping that will be transformed into a URI query
     *            string.
     *
     * @return A new URI value that combines the given URI and the constructed query
     *         string.
     * @throws URISyntaxException
     */
     ...
     * Given a Key / Value mapping create and append a URI query value that
     * represents the mapped entries, return the newly updated URI that contains the
     * value of the given URI and the appended query value. Each entry in the query
     ...
     *            string.
     * @param optionPrefix
     *            A string value that when not null or empty is used to prefix each
     *            query option key.
     *
     * @return A new URI value that combines the given URI and the constructed query
     *         string.
     * @throws URISyntaxException
     */
    public static URI applyParameters(URI uri, Map<String, String> queryParameters, String optionPrefix)
            throws URISyntaxException {
    ...
    /**
     * Removes any URI query from the given uri and return a new URI that does not
     * contain the query portion.
     *
     * @param uri
    ...
    /**
     * Creates a URI with the given query, removing an previous query value from the
     * given URI.
     *
     * @param uri
     ...
     *
     * @return a new URI that is a combination of the original URI and the given
     *         query string.
     * @throws URISyntaxException
     */
    ...
    /**
     * Given a composite URI, parse the individual URI elements contained within
     * that URI and return a CompsoteData instance that contains the parsed URI
     * values.
     *
     * @param uri
    ...
    /**
     * Given a string and a position in that string of an open parend, find the
     * matching close parend.
     *
     * @param str
     *            The string to be searched for a matching parend.
     * @param first
     *            The index in the string of the opening parend whose close value is
     *            to be searched.
     *
     * @return the index in the string where the closing parend is located.
     * @throws URISyntaxException
     *             fi the string does not contain a matching parend.
     */
    ...
    /**
     * Given a composite URI and a CompositeData instance and the scheme specific
     * part extracted from the source URI, parse the composite URI and populate the
     * CompositeData object with the results. The source URI is used only for
     * logging as the ssp should have already been extracted from it and passed
     * here.
     *
     * @param uri
     *            The original source URI whose ssp is parsed into the composite
     *            data.
     * @param rc
     *            The CompsositeData instance that will be populated from the given
     *            ssp.
     * @param ssp
     *            The scheme specific part from the original string that is a
     *            composite or one or more URIs.
     *
     * @throws URISyntaxException
     */
    ...
    /**
     * Given the inner portion of a composite URI, split and return each inner URI
     * as a string element in a new String array.
     *
     * @param str
     ...
     * <p>
     * 判断value是否以prefix为开始，若是则返回截取prefix后面的部分，若不是则返回value
     * 
     * @param value
     ...
     *
     * @return either the original string or a new string minus the supplied prefix
     *         if present.
     */
    ...
    /**
     * Strip(去掉) a URI of its scheme element.
     * <p>
     * 去掉URI中的scheme部分，相当于返回//之后的部分
     *
     * @param uri
    ...
    /**
     * Given a key / value mapping, create and return a URI formatted query string
     * that is valid and can be appended to a URI.
     * <p>
     * 将参数Map转换为URI中参数的形式
     *
     * @param options
     ...
     *
     * When the query options of a URI are applied to certain objects the used
     * portion of the query options needs to be removed and replaced with those that
     * remain so that other parts of the code can attempt to apply the remainder or
     * give an error is unknown values were given. This method is used to update a
     * URI with those remainder values.
     *
     * @param originalURI
     *            The URI whose current parameters are remove and replaced with the
     *            given remainder value.
     * @param params
     *            The URI params that should be used to replace the current ones in
     *            the target.
     *
     * @return a new URI that matches the original one but has its query options
    ...
    /**
     * Given a URI value create and return a new URI that matches the target one but
     * with the scheme value supplied to this method.
     * <p>
     * 重新生成一个新的URI，使用新的scheme，其余不变
     *
     * @param bindAddr
     ...
     *
     * @return a new URI that is a copy of the original except that its scheme
    ...
    /**
     * Examine the supplied string and ensure that all parends appear as matching
     * pairs.
     * <p>
     * 检查字符串中的()括号是不是成对出现的，是返回true，否返回false
     * 
     * @param str
